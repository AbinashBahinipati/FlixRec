import { getSeriesDetails, getWatchmodeImageUrl } from "@/lib/watchmode";
  ...
  if (!series) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-center p-6">
        <h1 className="text-3xl font-bold text-white mb-4">Series not found</h1>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <div className="relative w-full h-[50vh] sm:h-[70vh]">
        <Image
          src={getWatchmodeImageUrl(series.backdrop, "original")}
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="hidden md:block flex-shrink-0 w-64 rounded-xl overflow-hidden shadow-2xl border border-white/10">
            <div className="relative aspect-[2/3] w-full">
              <Image
                src={getWatchmodeImageUrl(series.poster)}
                alt={series.name || series.title}
                fill
                className="object-cover"
              />
          ...
          <div className="flex-1">
            <h1 className="text-4xl sm:text-5xl font-bold mb-4">{series.name || series.title}</h1>
            ...
            <div className="flex items-center gap-4 text-sm text-gray-300 font-medium mb-6">
              ...
              <span>{series.year || "Unknown"}</span>
              <span className="flex items-center gap-1">
                <span className="text-yellow-500">★</span>
                {series.user_rating ? series.user_rating.toFixed(1) : "NR"}
              </span>
              <span>{series.genre_names?.join(" • ") || "Unknown Genre"}</span>
            </div>
        ...
        {series.similar?.results?.length > 0 && (
          <div className="mt-8">
            <ContentRow 
              ...
              items={series.similar.results.map(mapToCard)} 
            />
          </div>
        )}
      </div>
